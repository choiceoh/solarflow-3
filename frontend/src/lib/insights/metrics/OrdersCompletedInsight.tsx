...
export function OrdersCompletedInsight() {
  const { dashboard, loading } = useOrderDashboard({ status: 'completed' })
  ...
  return (
    <InsightShell
      title="완료 수주"
      subtitle="status=completed 수주의 추세 · 거래처/제조사/관리구분 분해 (전량 출고)"
      unit="건"
      tone="pos"
      backTo="/orders"
