  ...
  const metrics: SalesMetric[] =
    activeTab === "outbound"
      ? [
          ...
          {
            lbl: "활성 출고",
            v: String(outboundDash?.totals.active_count ?? 0),
            numericValue: outboundDash?.totals.active_count ?? 0,
            formatter: fmtCount,
            u: "건",
            sub: "처리 중",
            tone: "info",
            spark: (outboundDash?.trend24 ?? []).slice(-6).map((p) => p.count),
            metricId: "outbound.active",
          },
          {
            lbl: "취소 대기",
            v: String(outboundDash?.totals.cancel_pending_count ?? 0),
            numericValue: outboundDash?.totals.cancel_pending_count ?? 0,
            formatter: fmtCount,
            u: "건",
            sub: "승인 대기",
            tone: (outboundDash?.totals.cancel_pending_count ?? 0) > 0 ? "warn" : "ink",
            metricId: "outbound.cancel_pending",
          },
          {
            lbl: "취소 완료",
            v: String(outboundDash?.totals.cancelled_count ?? 0),
            numericValue: outboundDash?.totals.cancelled_count ?? 0,
            formatter: fmtCount,
            u: "건",
            sub: "필터 기준",
            tone: "ink",
            metricId: "outbound.cancelled",
          },
          {
            lbl: "매출 미등록",
            v: String(saleUnregisteredCount),
            numericValue: saleUnregisteredCount,
            formatter: fmtCount,
            u: "건",
            sub: "출고 후 미연결",
            tone: saleUnregisteredCount > 0 ? "warn" : "pos",
      ...
      : activeTab === "sales"
        ? [
            ...
            {
              lbl: "공급가",
              v: fmtEok(saleDash?.totals.supply_amount_sum ?? 0),
              numericValue: saleDash?.totals.supply_amount_sum ?? 0,
              formatter: fmtEok,
              u: "억",
              sub: "VAT 별도",
              tone: "ink",
            ...
            },
            {
              ...
              sub: "발행 완료",
              tone: "pos",
              metricId: "sales.invoice_issued",
            },
            {
              lbl: "ERP 미마감",
              v: String(erpOpenCount),
              numericValue: erpOpenCount,
              formatter: fmtCount,
              u: "건",
              sub: "마감 작업 필요",
              tone: erpOpenCount > 0 ? "warn" : "pos",
              metricId: "sales.erp_open",
            },
          ]
        : activeTab === "receipts"
          ? [
              ...
              {
                lbl: "수금 미완료",
                v: String(receiptOpenSaleCount),
                numericValue: receiptOpenSaleCount,
                formatter: fmtCount,
                u: "건",
                sub: "매출채권 잔존",
                tone: receiptOpenSaleCount > 0 ? "warn" : "pos",
                metricId: "receipts.open_sale",
              },
              {
                lbl: "평균 입금",
                v: receiptCount > 0 ? fmtEok(receiptTotal / receiptCount) : "0.0",
                numericValue: receiptCount > 0 ? receiptTotal / receiptCount : 0,
                formatter: fmtEok,
                u: "억",
                sub: "1건당 평균",
                tone: "info",
                metricId: "receipts.avg_amount",
              },
            ]
          : activeTab === "matching"
            ? [
            ...
            : [
                ...
                {
                  lbl: "신규 접수",
                  v: String(orderDash?.totals.received_count ?? 0),
                  numericValue: orderDash?.totals.received_count ?? 0,
                  formatter: fmtCount,
                  u: "건",
                  sub: "출고 대기",
                  tone: "info",
                  metricId: "orders.received",
                },
                {
                  lbl: "완료 수주",
                  v: String(orderDash?.totals.completed_count ?? 0),
                  numericValue: orderDash?.totals.completed_count ?? 0,
                  formatter: fmtCount,
                  u: "건",
                  sub: "전량 출고",
                  tone: "pos",
                  metricId: "orders.completed",
                },
                {
                  lbl: "취소 수주",
                  v: String(orderDash?.totals.cancelled_count ?? 0),
                  numericValue: orderDash?.totals.cancelled_count ?? 0,
                  formatter: fmtCount,
                  u: "건",
                  sub: "필터 기준",
                  tone: "ink",
                ...
                },
                {
                  lbl: "충당 부족",
                  v: String(
                    Object.values(orderRiskByOrder).filter((r) => r.risk === "shortage").length,
                  ),
                  numericValue: Object.values(orderRiskByOrder).filter((r) => r.risk === "shortage")
                    .length,
                  formatter: fmtCount,
                  u: "건",
                  sub: "재고+미착 부족",
                  tone: "warn",
                  metricId: "orders.shortage",
                },
                {
                  lbl: "전체 평단",
                  v: (orderDash?.totals.avg_unit_price_wp ?? 0).toFixed(1),
                  numericValue: orderDash?.totals.avg_unit_price_wp ?? 0,
                  formatter: fmtFixed1,
                  u: "원/Wp",
                  sub: "필터 기준 전체",
                  tone: "ink",
                  metricId: "orders.avg_unit_price_wp",
                },
              ]
