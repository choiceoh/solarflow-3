...
export default function WarehouseEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchWithAuth<Warehouse>(`/api/v1/warehouses/${id}`);
  ...
  const handleSubmit = async (data: WarehouseFormData) => {
    ...
    setSaving(true);
    try {
      await fetchWithAuth(`/api/v1/warehouses/${id}`, { method: 'PUT', body: JSON.stringify(data) });
      navigate(BACK_TO);
    } finally {
      setSaving(false);
    }
  ...
  if (loadError || !editData) {
    return (
      <EditPageShell eyebrow="DATA · 창고" title="창고 수정" backTo={BACK_TO} formId={FORM_ID}>
        <p className="text-sm text-destructive">창고 정보를 불러올 수 없습니다. {loadError ?? ''}</p>
      </EditPageShell>
    );
  }
  ...
  return (
    <EditPageShell
      eyebrow="DATA · 창고"
      title={`창고 수정 — ${editData.warehouse_name}`}
      backTo={BACK_TO}
      formId={FORM_ID}
      saving={saving}
    >
      <WarehouseFormBody formId={FORM_ID} editData={editData} onSubmit={handleSubmit} />
    </EditPageShell>
  );
}
