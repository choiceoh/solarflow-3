...
export default function BankingPage() {
  ...
  const pageSub =
    activeTab === "maturity"
      ? `${alertRows.length}건 · 30일 이내`
      : activeTab === "changes"
        ? `${limitChanges.length}건 · 승인한도 변경`
        : activeTab === "demand"
          ? "PO 기반 한도 소요 전망"
    ...
    <div className="sf-page">
      <div className="sf-procurement-layout">
        <section className="sf-procurement-main">
          ...
          <CardB title={pageTitle} sub={pageSub} right={bankingCardControls} headerless>
        ...
        <aside className="sf-procurement-rail card">
          ...
          <RailBlock title="은행별 사용률" count="M$">
            {allLimitRows.slice(0, 6).map((row, index) => (
              <div
                key={`${row.bank_id ?? row.bank_name}-${index}`}
              ...
              >
                <div className="flex items-baseline justify-between text-[11.5px]">
                  ...
                  <span className="mono text-[var(--ink-3)]">
                    {fmtUsdM(row.used)} / {fmtUsdM(row.lc_limit_usd)}
                  </span>
                </div>
                <div className="mt-1 h-1 overflow-hidden rounded bg-[var(--line)]">
                  <div
                    className="h-full bg-[var(--solar-2)]"
                    style={{ width: `${Math.min(100, row.usage_rate)}%` }}
                  />
                </div>
              </div>
            ))}
          ...
          <RailBlock title="만기 임박" count={alertRows.length} last>
            {alertRows.slice(0, 5).map((alert, index) => (
              <div
                key={`${alert.lc_id}-${alert.maturity_date}-${index}`}
                className={`grid grid-cols-[auto_1fr_auto] gap-2 py-2 text-[11.5px] ${index ? "border-t border-[var(--line)]" : ""}`}
              >
                <span className="mono font-bold text-[var(--warn)]">D-{alert.days_remaining}</span>
                <span className="mono truncate text-[var(--ink-2)]">
                  {alert.lc_number ?? alert.lc_id.slice(0, 8)}
                </span>
            ...
            <Sparkline
              data={[62, 64, 65, 67, 69, 70, totalUsageRate]}
              w={220}
              h={34}
              color="var(--solar-2)"
              area
      ...
      <LimitChangeForm
        open={lcFormOpen}
        onOpenChange={setLcFormOpen}
        onSubmit={handleCreateLimitChange}
      />
    </div>
  )
}
