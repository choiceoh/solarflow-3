...
import type {
  BaroPurchaseHistoryItem,
  BaroPurchaseHistorySummary,
  BaroPurchaseInboundType,
  BaroPurchaseStatus,
...
function hasCost(row: BaroPurchaseHistoryItem): boolean {
  return (
    row.unit_price_krw_wp != null ||
    row.unit_price_usd_wp != null ||
    row.estimated_amount_krw != null ||
    row.estimated_amount_usd != null
  )
}
...
export default function BaroPurchaseHistoryPage() {
  ...
  const [serverSummary, setServerSummary] = useState<BaroPurchaseHistorySummary | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (inboundFilter !== "__all__") summaryParams.set("inbound_type", inboundFilter)
      if (costFilter !== "all") summaryParams.set("cost_filter", costFilter)
      const [list, summary] = await Promise.all([
        fetchWithAuth<BaroPurchaseHistoryItem[]>(
          `/api/v1/baro/purchase-history?${params.toString()}`,
        ),
        fetchWithAuth<BaroPurchaseHistorySummary>(
          `/api/v1/baro/purchase-history/summary?${summaryParams.toString()}`,
        ),
      ])
      setRows(list)
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      ...
      if (costFilter === "krw")
        return row.unit_price_krw_wp != null || row.estimated_amount_krw != null
      if (costFilter === "usd")
        return row.unit_price_usd_wp != null || row.estimated_amount_usd != null
  ...
  const localSummary = useMemo(() => {
    ...
    const krwRows = filteredRows.filter(
      (row) => row.unit_price_krw_wp != null && row.capacity_kw > 0,
    )
    const usdRows = filteredRows.filter(
      (row) => row.unit_price_usd_wp != null && row.capacity_kw > 0,
    )
    const weightedKrw = krwRows.reduce(
      (sum, row) => sum + Number(row.unit_price_krw_wp) * row.capacity_kw,
      0,
    )
    const weightedKw = krwRows.reduce((sum, row) => sum + row.capacity_kw, 0)
    const weightedUsd = usdRows.reduce(
      (sum, row) => sum + Number(row.unit_price_usd_wp) * row.capacity_kw,
      0,
    )
  ...
  const summary = useMemo(() => {
    if (query.trim() || !serverSummary) return localSummary
    return {
      totalQty: serverSummary.total_quantity,
      totalKw: serverSummary.total_capacity_kw,
      avgKrwWp: serverSummary.average_krw_wp ?? null,
      avgUsdWp: serverSummary.average_usd_wp ?? null,
      totalKrw: serverSummary.total_amount_krw,
      totalUsd: serverSummary.total_amount_usd,
      costCovered: serverSummary.cost_covered_count,
      totalRows: serverSummary.total,
    }
  }, [localSummary, query, serverSummary])
  ...
  const columns: Column<BaroPurchaseHistoryItem>[] = [
      ...
      render: (row) => (
        <span className="flex flex-col">
          <span className="font-medium tabular-nums">
            {formatDate(row.purchase_date ?? row.actual_arrival ?? row.eta ?? "")}
          </span>
      ...
      render: (row) => (
        <span className="flex min-w-[230px] flex-col">
          <span className="font-medium">
            ...
            {row.product_code ? (
              <span className="ml-1 text-xs text-muted-foreground">{row.product_code}</span>
            ) : null}
          </span>
    ...
    },
    {
      ...
      render: (row) => (
        <span className="flex flex-col text-right tabular-nums">
          ...
          <span className="text-[11px] text-muted-foreground">
            {formatUsdWp(row.unit_price_usd_wp)}
          </span>
        </span>
      ),
    },
    {
      ...
      render: (row) => (
        <span className="flex flex-col text-right tabular-nums">
          <span className="font-medium">
            {row.estimated_amount_krw != null ? formatKRW(row.estimated_amount_krw) : "—"}
          </span>
          <span className="text-[11px] text-muted-foreground">
            {row.estimated_amount_usd != null ? formatUSD(row.estimated_amount_usd) : row.currency}
          </span>
        </span>
      ),
    },
    {
      ...
      render: (row) => (
        <span className="flex min-w-[180px] flex-col text-[13px]">
          ...
          <span className="text-muted-foreground">
            {row.warehouse_name ?? row.port ?? "도착지 미정"}
          </span>
        </span>
      ),
    },
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="rounded-md border bg-card px-4 py-3">
          ...
          <div className="mt-1 text-lg font-semibold tabular-nums">
            {formatNumber(summary.totalQty)}장
          </div>
        </div>
        <div className="rounded-md border bg-card px-4 py-3">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
          ...
          <div className="mt-1 text-lg font-semibold tabular-nums">
            {formatCapacity(summary.totalKw)}
          </div>
        </div>
        <div className="rounded-md border bg-card px-4 py-3">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
          ...
          <div className="mt-1 text-lg font-semibold tabular-nums">
            {formatKrwWp(summary.avgKrwWp ?? undefined)}
          </div>
        </div>
        <div className="rounded-md border bg-card px-4 py-3">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
          ...
          <div className="mt-1 text-lg font-semibold tabular-nums">
            {summary.costCovered.toLocaleString("ko-KR")} /{" "}
            {summary.totalRows.toLocaleString("ko-KR")}
          </div>
        </div>
      </div>
          ...
          <Select value={inboundFilter} onValueChange={(v) => setInboundFilter(v as InboundFilter)}>
            <SelectTrigger className="h-8 w-36 text-xs">
              <span className="flex-1 truncate text-left">
                {inboundFilter === "__all__"
                  ? "전체 매입처"
                  : BARO_PURCHASE_INBOUND_LABEL[inboundFilter]}
              </span>
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__all__">전체 매입처</SelectItem>
              {(Object.keys(BARO_PURCHASE_INBOUND_LABEL) as BaroPurchaseInboundType[]).map(
                (type) => (
            ...
            <TableRow className="bg-muted/20 hover:bg-muted/20">
              <TableCell>
                <span className="whitespace-nowrap font-medium">
                  합계 · 필터 결과 {summary.totalRows.toLocaleString("ko-KR")}건
                </span>
              </TableCell>
              ...
              <TableCell className="text-right tabular-nums">
                <span className="whitespace-nowrap">
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    {" "}
                    · {formatUsdWp(summary.avgUsdWp ?? undefined)}
                  </span>
                </span>
              </TableCell>
              <TableCell className="text-right tabular-nums">
                <span className="whitespace-nowrap">
                  <span className="font-medium">
                    {summary.totalKrw > 0 ? formatKRW(summary.totalKrw) : "—"}
                  </span>
                  <span className="text-[11px] text-muted-foreground">
                    {" "}
