...
export type BaroPurchaseStatus =
  | "scheduled"
  | "shipping"
  | "arrived"
  | "customs"
...
export interface BaroPurchaseHistorySummary {
  total: number
  shipment_count: number
  total_quantity: number
  total_capacity_kw: number
  average_krw_wp?: number
  average_usd_wp?: number
  total_amount_krw: number
  total_amount_usd: number
  cost_covered_count: number
  by_inbound_type: Partial<Record<BaroPurchaseInboundType | string, number>>
  by_status: Partial<Record<BaroPurchaseStatus | string, number>>
}
