...
export default function OrdersPage() {
  ...
  const metrics: SalesMetric[] =
    ...
    activeTab === 'sales' ? [
      ...
      { lbl: '평균 단가', v: sales.length ? Math.round(sales.reduce((sum, sale) => sum + (sale.unit_price_ea ?? (sale.unit_price_wp ?? 0) * (sale.spec_wp ?? 0)), 0) / sales.length).toLocaleString() : '0', u: '₩/장', sub: '필터 기준', tone: 'ink' },
    ] :
    activeTab === 'receipts' ? [
    ...
    activeTab === 'matching' ? [
      ...
      { lbl: '평균 단가', v: visibleOrders.length ? Math.round(visibleOrders.reduce((sum, order) => sum + (order.unit_price_ea ?? (order.unit_price_wp ?? 0) * (order.spec_wp ?? 0)), 0) / visibleOrders.length).toLocaleString() : '0', u: '₩/장', sub: '수주 기준', tone: 'pos' },
    ];
            ...
            <>
              ...
              <RailBlock title="단가 흐름" last>
                ...
                <div className="mono mt-2 flex justify-between text-[10.5px] text-[var(--ink-3)]">
                  <span>평균 <span className="font-bold text-[var(--ink)]">{visibleOrders.length ? Math.round(visibleOrders.reduce((sum, order) => sum + (order.unit_price_ea ?? (order.unit_price_wp ?? 0) * (order.spec_wp ?? 0)), 0) / visibleOrders.length).toLocaleString() : '0'}</span> ₩/장</span>
