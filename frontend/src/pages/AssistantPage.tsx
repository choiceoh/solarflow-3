...
export function ChatBox({
  ...
  const [proposalStatuses, setProposalStatuses] = useState<
    Map<string, { status: ProposalStatus; errorMessage?: string }>
  >(new Map())
  ...
  const updateProposalStatus = (
    id: string,
    patch: { status: ProposalStatus; errorMessage?: string },
  ) => {
  ...
  const onConfirm = async (prop: ProposalState, overridePayload?: unknown) => {
    ...
    try {
      await fetchWithAuth(`/api/v1/assistant/proposals/${prop.id}/confirm`, {
        method: "POST",
        body:
          overridePayload === undefined ? undefined : JSON.stringify({ payload: overridePayload }),
      })
              ...
              {messagesWithProposals.map(({ message, proposals }) => (
                <div
                  ...
                  className={cn(
                    ...
                    highlightedKey === message.id &&
                      "bg-[var(--sf-solar)]/10 ring-2 ring-[var(--sf-solar)]/40",
                  )}
                >
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 text-center text-base text-muted-foreground">
      ...
      <div className="text-sm opacity-70">
        DB 조회·작성·OCR 도구가 활성화됩니다. 쓰기는 카드의 [저장] 클릭 시에만 반영됩니다.
      </div>
    </div>
  )
}
