    ...
    <div className="space-y-3 text-xs">
      ...
      <div className="rounded-md border bg-card p-3 space-y-1">
        ...
        {config.page?.description && (
          <div className="text-[11px] text-muted-foreground">{config.page.description}</div>
        )}
      </div>
      ...
      {config.metrics && config.metrics.length > 0 && (
        <div className="space-y-1">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">메트릭</p>
      ...
      {config.filters && config.filters.length > 0 && (
        <div className="space-y-1">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">필터</p>
        ...
        <div className="rounded-md border bg-card overflow-hidden">
          <div className="bg-muted/30 px-3 py-1.5 grid gap-2 border-b" style={{ gridTemplateColumns: `repeat(${Math.min(config.columns?.length ?? 1, 6)}, minmax(0, 1fr))` }}>
            {(config.columns ?? []).slice(0, 6).map((c, i) => (
              <div key={i} className="text-[10px] font-medium truncate" title={c.key} style={{ textAlign: c.align }}>
                {c.label}
          ...
          {[0, 1, 2].map((row) => (
            <div key={row} className="px-3 py-1.5 grid gap-2 border-b last:border-b-0" style={{ gridTemplateColumns: `repeat(${Math.min(config.columns?.length ?? 1, 6)}, minmax(0, 1fr))` }}>
              {(config.columns ?? []).slice(0, 6).map((c, i) => (
                <div key={i} className="text-[10px] text-muted-foreground/60" style={{ textAlign: c.align }}>—</div>
              ))}
            </div>
          ))}
        </div>
        {(config.columns?.length ?? 0) > 6 && (
          <p className="text-[10px] text-muted-foreground italic">+ {(config.columns?.length ?? 0) - 6} 컬럼 더 (좁아서 6개만 표시)</p>
        )}
      </div>
      ...
      {config.actions && config.actions.length > 0 && (
        <div className="space-y-1">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">액션</p>
          <div className="flex flex-wrap gap-1.5">
            {config.actions.map((a, i) => (
              <div key={i} className={`rounded border px-2 py-1 text-[10px] ${a.variant === 'primary' ? 'bg-foreground text-background' : 'bg-background'}`}>
                {a.label}
              </div>
            ))}
          </div>
  ...
  return (
    <div className="space-y-3 text-xs">
      ...
      <div className="rounded-md border bg-card p-3 space-y-1">
        ...
        <div className="text-[10px] text-muted-foreground">신규 등록 다이얼로그 미리보기</div>
      </div>
      ...
      {(config.sections ?? []).map((sec, sIdx) => (
        <div key={sIdx} className={`rounded-md border bg-card p-3 ${sec.tone ? TONE_BG[sec.tone] ?? '' : ''}`}>
          {sec.title && (
            <div className="text-[11px] font-semibold mb-2 pb-1 border-b border-current/20">{sec.title}</div>
          )}
      ...
      <div className="flex justify-end gap-2 pt-2">
        <div className="rounded border bg-background px-3 py-1 text-[11px]">취소</div>
        <div className="rounded bg-foreground text-background px-3 py-1 text-[11px]">저장</div>
      </div>
...
function FieldInputMockup({ field }: { field: FieldConfig }) {
  const baseClass = "h-7 w-full rounded border border-input bg-background/50 px-2 text-[11px] flex items-center text-muted-foreground/60";
  const ph = field.placeholder ?? '';
  switch (field.type) {
    case 'switch':
      return <div className="flex items-center gap-1.5">
        <div className="h-4 w-7 rounded-full bg-muted-foreground/20" />
        <span className="text-[10px] text-muted-foreground">on / off</span>
      </div>;
    case 'textarea':
