  ...
  return (
    <>
      ...
      <div className="rounded-lg border bg-card p-5 space-y-3">
        <p className="text-sm font-medium text-muted-foreground uppercase tracking-wide">역할 안내</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
          <div className="flex gap-2 items-start">
            <span className={`mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[11px] font-medium ${ROLE_BADGE_VARIANT.admin}`}>시스템관리자</span>
            <span className="text-muted-foreground">전체 기능 + 사용자 관리·설정</span>
          </div>
          <div className="flex gap-2 items-start">
            <span className={`mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[11px] font-medium ${ROLE_BADGE_VARIANT.operator}`}>운영팀</span>
            <span className="text-muted-foreground">전체 기능 (설정 제외)</span>
          </div>
          <div className="flex gap-2 items-start">
            <span className={`mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[11px] font-medium ${ROLE_BADGE_VARIANT.executive}`}>경영진</span>
            <span className="text-muted-foreground">전체 조회 (민감정보 포함, 입력 없음)</span>
          </div>
          <div className="flex gap-2 items-start">
            <span className={`mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[11px] font-medium ${ROLE_BADGE_VARIANT.manager}`}>본부장</span>
            <span className="text-muted-foreground">재고·가용재고 조회 (민감정보 제외)</span>
          </div>
          <div className="flex gap-2 items-start">
            <span className={`mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[11px] font-medium ${ROLE_BADGE_VARIANT.viewer}`}>조회</span>
        ...
        {loading ? (
          ...
          <div className="divide-y">
            {users.map((u) => {
              ...
              return (
                <div key={u.user_id} className={`flex items-center gap-4 px-5 py-4 ${!u.is_active ? 'opacity-50' : ''}`}>
                  {/* 사용자 정보 */}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-base font-medium truncate">{u.name}</p>
                      <span className={`shrink-0 rounded px-1.5 py-0.5 text-[11px] font-medium ${ROLE_BADGE_VARIANT[u.role] ?? 'bg-gray-100 text-gray-600'}`}>
                        {ROLE_LABELS[u.role] ?? u.role}
                      </span>
                      {isSelf && (
                        <span className="shrink-0 rounded bg-emerald-100 text-emerald-700 px-1.5 py-0.5 text-[11px] font-medium">본인</span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0" title={isSelf ? '본인 계정은 비활성화할 수 없습니다' : undefined}>
                    <Switch
                      ...
                      disabled={isSaving || isSelf}
                    />
                    <span className="text-sm text-muted-foreground w-12">{u.is_active ? '활성' : '비활성'}</span>
                  </div>
