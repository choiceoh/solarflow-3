...
export default function LCMaturityTable({ alertData }: Props) {
  ...
  if (alerts.length === 0) {
    return (
      <p className="text-sm text-muted-foreground text-center py-6">30일 이내 만기 LC가 없습니다</p>
    )
  }

  const handleExpand = (rowKey: string, lcId: string) => {
  ...
  return (
    <Table>
      ...
      <TableBody>
        {alerts.map((a, index) => {
          const rowKey = `${a.lc_id}-${a.maturity_date}-${a.po_number ?? "po"}-${index}`
          return (
            <Fragment key={rowKey}>
              <TableRow
                className="cursor-pointer hover:bg-muted/50"
                onClick={() => handleExpand(rowKey, a.lc_id)}
              >
