...
const STATUS_STYLE: Record<string, string> = {
  pending: "sf-tone-warn",
  confirmed: "sf-tone-pos",
  hold: "sf-tone-info",
  cancelled: "sf-tone-muted",
}
...
function AllocSubTable({
  ...
  if (allocs.length === 0) {
    return <p className="text-xs text-muted-foreground px-2 py-1">등록된 내역 없음</p>
  }
          ...
          return (
            ...
            >
              ...
              <td className="p-2">
                ...
                {isFreeSpare && <span className="sf-status-pill sf-tone-warn">무상스페어</span>}
              </td>
              ...
              <td className="p-2 text-center">
                <span className={`sf-status-pill ${STATUS_STYLE[a.status] ?? "sf-tone-muted"}`}>
...
function IncomingTable({
  ...
  const allocCountMap = useMemo(() => {
    ...
    for (const it of incoming) {
      map.set(
        it.product_id,
        allocations.filter((a) => a.product_id === it.product_id && a.source_type === "incoming")
          .length,
      )
    }
    return map
  ...
  const { sorted, headerProps } = useSort<InventoryItem>(incoming, (it, f) => {
    switch (f) {
      ...
      case "incoming_kw":
        return it.incoming_kw ?? 0
      case "incoming_reserved_kw":
        return it.incoming_reserved_kw ?? 0
      case "available_incoming_kw":
        return it.available_incoming_kw ?? 0
      case "alloc_count":
        return allocCountMap.get(it.product_id) ?? 0
      default:
        return null
    }
  })
      ...
      <table className="w-full text-xs">
        <thead className="bg-muted/50">
          <tr>
            ...
            <SortableTH
              {...headerProps("product_code")}
              className="p-2 font-medium text-muted-foreground"
            >
              품목
            </SortableTH>
            <SortableTH
              {...headerProps("incoming_kw")}
              align="right"
              className="p-2 font-medium text-muted-foreground"
            >
            ...
            <SortableTH
              {...headerProps("incoming_reserved_kw")}
              align="right"
              className="p-2 font-medium text-muted-foreground"
            >
              <span className="inline-flex items-center gap-1">
            ...
            <SortableTH
              {...headerProps("available_incoming_kw")}
              align="right"
              className="p-2 font-medium text-muted-foreground"
            >
              <span className="inline-flex items-center gap-1">
            ...
            return (
              <Fragment key={item.product_id}>
                ...
                >
                    ...
                    {item.incoming_reserved_kw > 0 ? (
                      <>
                        <div className="font-semibold text-red-500">
                          {fmtKw(item.incoming_reserved_kw)}
                        </div>
                        <div className="text-[10px] text-muted-foreground">
                          {kwToEa(item.incoming_reserved_kw, item.spec_wp).toLocaleString("ko-KR")}{" "}
                          EA
                        </div>
                      </>
                    ) : (
                  ...
                  <td className="p-2 text-right tabular-nums">
                    <div
                      className={`font-semibold ${item.available_incoming_kw > 0 ? "text-green-600" : "text-muted-foreground"}`}
                    >
        ...
        <tfoot>
          <tr className="border-t bg-muted/20">
            ...
            <td className="p-2 text-right tabular-nums font-medium text-yellow-600">
              {fmtKw(totals.incoming)}
            </td>
            <td className="p-2 text-right tabular-nums font-medium text-red-500">
              {fmtKw(totals.reserved)}
            </td>
            <td className="p-2 text-right tabular-nums font-medium text-green-600">
              {fmtKw(totals.available)}
            </td>
            <td className="p-2 text-right tabular-nums font-medium">
              {totals.allocCount.toLocaleString("ko-KR")}건
            </td>
            <td />
          </tr>
        </tfoot>
