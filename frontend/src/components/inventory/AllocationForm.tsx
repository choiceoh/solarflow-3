...
function isLinkedFreeSpare(main: InventoryAllocation, candidate: InventoryAllocation): boolean {
  ...
  if (candidate.company_id !== main.company_id || candidate.product_id !== main.product_id)
    return false
...
function SiteCombobox({
  ...
  const typeLabel =
    siteType === "own" ? (
      <span className="sf-status-pill sf-tone-solar">자체</span>
    ) : (
      <span className="sf-status-pill sf-tone-warn">EPC</span>
    )
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1 w-full rounded-lg border border-border bg-popover shadow-md overflow-hidden">
          ...
          {!creating ? (
            ...
            >
              <Plus className="size-3.5" />새 현장 직접 등록...
            </button>
          ) : (
...
function Txt({ text, placeholder = "선택" }: { text: string; placeholder?: string }) {
  return (
    <span
      className={cn("flex flex-1 text-left truncate", !text && "text-muted-foreground")}
...
export default function AllocationForm({
  ...
  const [purpose, setPurpose] = useState<
    "sale" | "construction_own" | "construction_epc" | "other"
  >("sale")
    ...
    fetchWithAuth<BLShipment[]>(`/api/v1/bls?manufacturer_id=${mfgId}`)
      .then((list) => {
        const relevant = (list ?? []).filter((b) =>
          ["scheduled", "shipping", "arrived", "customs", "completed", "erp_done"].includes(
            b.status,
          ),
        )
        setBls(relevant)
  ...
  useEffect(() => {
    if (!productId || bls.length === 0) {
      setBlPriceMap(new Map())
      setBlUsdPriceMap(new Map())
      return
    }
    Promise.all(
      bls.map((bl) =>
  ...
  useEffect(() => {
    ...
    if (editData) {
      ...
      setExpectedPrice(
        editData.expected_price_per_wp != null ? String(editData.expected_price_per_wp) : "",
      )
      setFreeSpareQty(
        editData.free_spare_qty && editData.free_spare_qty > 0
          ? String(editData.free_spare_qty)
          : "",
      )
      setLinkedFreeSpares([])
  ...
  const selectedBl = useMemo(() => bls.find((b) => b.bl_id === selectedBlId), [bls, selectedBlId])
  ...
  /* EA 입력값 표시 (천단위 콤마) */
  const qtyDisplayEa =
    qtyMode === "ea" && qtyRaw ? parseInt(qtyRaw, 10).toLocaleString("ko-KR") : ""

  /* 저장 */
  const handleSave = async () => {
    setError("")
    if (!effectiveCompanyId || effectiveCompanyId === "all") {
      setError("좌측 상단에서 법인을 먼저 선택해주세요")
      return
    }
    if (!productId) {
      setError("품목을 선택해주세요")
    ...
    const base = {
      ...
      expected_price_per_wp:
        !Number.isNaN(parsedPrice) && parsedPrice > 0 ? parsedPrice : undefined,
        ...
        if (spareEa > 0) {
          ...
          await Promise.all(
            duplicateSpares.map((spare) =>
              fetchWithAuth(`/api/v1/inventory/allocations/${spare.alloc_id}`, {
                method: "DELETE",
              }),
            ),
          )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl w-[95vw] max-h-[90vh] flex flex-col p-0 gap-0">
            ...
            <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2.5 space-y-2">
              ...
              <Select value={selectedCompanyId || "all"} onValueChange={(v) => setCompanyId(v)}>
                ...
                <SelectContent>
                  {companies.map((c) => {
                    const hasInv =
                      companyHasInv.size === 0 || companyHasInv.get(c.company_id) !== false
                    return (
                      <SelectItem key={c.company_id} value={c.company_id} disabled={!hasInv}>
                ...
                <div className="rounded-md bg-muted/40 px-3 py-2 text-xs grid grid-cols-3 gap-2">
                  <div className="text-center">
                    ...
                    <div className="font-semibold text-green-700">
                      {eaFromKw(selectedItem.available_kw, selectedItem.spec_wp).toLocaleString()}{" "}
                      EA
                    </div>
                    <div className="text-muted-foreground">
                      {(selectedItem.available_kw / 1000).toFixed(2)} MW
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="font-semibold text-blue-700">
                      {eaFromKw(
                        selectedItem.available_incoming_kw,
                        selectedItem.spec_wp,
                      ).toLocaleString()}{" "}
                      EA
                    </div>
                    <div className="text-muted-foreground">
                      {(selectedItem.available_incoming_kw / 1000).toFixed(2)} MW
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="font-semibold text-purple-700">
                      {eaFromKw(
                        selectedItem.total_secured_kw,
                        selectedItem.spec_wp,
                      ).toLocaleString()}{" "}
                      EA
                    </div>
                    <div className="text-muted-foreground">
                      {(selectedItem.total_secured_kw / 1000).toFixed(2)} MW
                    </div>
                  </div>
                </div>
              )}
                  ...
                  onClick={() => {
                    ...
                    if (selectedItem && inputEa > 0) {
                      if (nextMode === "ea") setQtyRaw(String(inputEa))
                        ...
                        setQtyRaw(((inputEa * selectedItem.spec_wp) / 1_000).toFixed(1))
                  ...
                  <SelectTrigger className="w-full">
                    <Txt
                      text={
                        selectedBl
                          ? (() => {
                              const modLabel = moduleLabel(
                                selectedItem?.manufacturer_name,
                                selectedItem?.spec_wp,
                              )
                              const dateLabel = selectedBl.actual_arrival
                                ? `입항일: ${selectedBl.actual_arrival.slice(0, 10)}`
                                : `ETA: ${selectedBl.eta?.slice(0, 10) ?? "—"}`
                              const stKo = ["completed", "erp_done"].includes(selectedBl.status)
                                ? "입고완료"
                                : statusLabel(selectedBl.inbound_type, selectedBl.status)
                  ...
                  (() => {
                    ...
                    return (
                      <div className="rounded border bg-blue-50 px-3 py-1.5 text-[10px] text-blue-700 flex flex-wrap gap-x-4 gap-y-1">
                        ...
                        {bl.exchange_rate && (
                          <span>환율: {bl.exchange_rate.toLocaleString("ko-KR")}</span>
                        )}
            ...
            {allocationPlan && selectedItem && (
              ...
              >
                ...
                {allocationPlan.stockEa > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      <span className="sf-status-pill sf-tone-pos">현재고</span>
                      <span className="font-medium">
                        {allocationPlan.stockEa.toLocaleString()} EA
                      </span>
                    </div>
                    <span className="text-muted-foreground text-xs">
                      {formatCapacityAuto((allocationPlan.stockEa * selectedItem.spec_wp) / 1000)}
                    </span>
                  </div>
                )}
                {allocationPlan.incomingEa > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      <span className="sf-status-pill sf-tone-info">미착품</span>
                      <span className="font-medium">
                        {allocationPlan.incomingEa.toLocaleString()} EA
                      </span>
                    </div>
                    <span className="text-muted-foreground text-xs">
                      {formatCapacityAuto(
                        (allocationPlan.incomingEa * selectedItem.spec_wp) / 1000,
                      )}
                    </span>
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 gap-3">
                {purpose === "sale" ? (
                  <>
                    ...
                    <div className="space-y-1.5">
                      <Label>
                        현장명 <span className="text-muted-foreground text-xs">(선택)</span>
                      </Label>
                      <Input
                        value={siteName}
                  ...
                  <>
                    ...
                    <div className="space-y-1.5">
                      <Label>
                        거래처명 <span className="text-muted-foreground text-xs">(선택)</span>
                      </Label>
                      <PartnerCombobox
                        partners={partners}
                        value={customerPartnerId}
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selectedItem &&
                    inputEa > 0 &&
                    expectedPrice &&
                    !Number.isNaN(parseFloat(expectedPrice)) &&
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selectedItem && freeSpareQty && parseInt(freeSpareQty, 10) > 0 && (
                    <div className="h-9 flex items-center rounded-md border bg-muted px-3 text-xs text-muted-foreground whitespace-nowrap">
                      {formatCapacityAuto(
                        (parseInt(freeSpareQty, 10) * selectedItem.spec_wp) / 1000,
                      )}
                    </div>
                  )}
                </div>
        ...
        <DialogFooter className="px-6 py-4 border-t shrink-0">
          ...
          <Button onClick={handleSave} disabled={saving || allocationPlan?.type === "insufficient"}>
