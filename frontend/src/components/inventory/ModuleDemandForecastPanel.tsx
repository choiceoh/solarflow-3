...
export default function ModuleDemandForecastPanel({
  companyId,
  inventoryItems,
  manufacturers,
}: Props) {
  ...
  const moduleOptions = useMemo(() => {
    ...
    for (const item of inventoryItems) {
      ...
      const key = moduleKey(
        item.spec_wp,
        item.module_width_mm,
        item.module_height_mm,
        itemCompanyId,
      )
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [demandList, saleList, companyList] = await Promise.all([
        fetchWithAuth<ModuleDemandForecast[]>(
          companyQueryUrl("/api/v1/module-demand-forecasts", companyId),
        ),
  ...
  const openNew = () => {
    setEditing(null)
    setForm({
      ...emptyForm(moduleOptions[0]?.key ?? ""),
      companyId: companyId === "all" ? (moduleOptions[0]?.companyId ?? "") : companyId,
    })
    setFormOpen(true)
  }
  ...
  const save = async () => {
    ...
    if (!option) {
      setError("모듈군을 선택해 주세요")
      return
    }
    if (!targetCompanyId) {
      setError("법인을 선택해 주세요")
      return
    }
    ...
    if (!Number.isFinite(requiredMw) || requiredMw <= 0) {
      setError("필요 용량은 양수여야 합니다")
      return
    }
  ...
  const remove = async (item: ModuleDemandForecast) => {
    ...
    if (!ok) return
    try {
      await fetchWithAuth(`/api/v1/module-demand-forecasts/${item.forecast_id}`, {
        method: "DELETE",
      })
      await load()
    } catch (err) {
  ...
  const forecastRows = useMemo(() => {
    ...
    for (const sale of sales) {
      ...
      const key = sale.product_id
        ? (productToModule.get(`${sale.product_id}:${sale.company_id ?? ""}`) ??
          productToModule.get(sale.product_id))
        : undefined
      if (!key) continue
      const kw =
        sale.capacity_kw ??
    ...
    const demandKeys = new Set(
      activeDemands.map((d) =>
        moduleKey(d.spec_wp, d.module_width_mm, d.module_height_mm, d.company_id),
      ),
    )
    ...
    return Array.from(allKeys)
      .map((key) => {
        ...
        const matchingDemand = activeDemands.filter(
          (d) => moduleKey(d.spec_wp, d.module_width_mm, d.module_height_mm, d.company_id) === key,
        )
        ...
        return {
          ...
          actionMonth,
          status:
            depletionMonth && actionMonth <= currentMonth
              ? "negotiate"
              : depletionMonth
                ? "watch"
                : "ok",
      ...
      .sort((a, b) => {
        if (a.status !== b.status)
          return a.status === "negotiate" ? -1 : b.status === "negotiate" ? 1 : 0
        return b.securedKw - a.securedKw
      })
  }, [companyId, demands, inventoryItems, moduleOptions, sales])

  const selectedManufacturerName =
    form.manufacturerId === "any"
      ? "제조사 무관"
      : shortMfgName(
          manufacturers.find((m) => m.manufacturer_id === form.manufacturerId)?.name_kr || "",
        )
  const selectedCompanyName =
    companies.find((company) => company.company_id === form.companyId)?.company_name || "법인 선택"
  const selectedModuleLabel =
    moduleOptions.find((option) => option.key === form.moduleKey)?.label || "모듈군 선택"
                  ...
                  <tr key={row.key} className="border-b last:border-b-0">
                    ...
                    <td className="px-3 py-2 text-center">
                      <span
                        className={cn(
                          "sf-status-pill",
                          row.status === "negotiate" && "sf-tone-neg",
                          row.status === "watch" && "sf-tone-warn",
                          row.status === "ok" && "sf-tone-pos",
                        )}
                      >
              ...
              <tfoot>
                <tr className="border-t bg-muted/20">
                  ...
                  <td className="px-3 py-2 text-right font-medium">
                    {formatKw(forecastTotals.secured)}
                  </td>
                  <td className="px-3 py-2 text-right font-medium">
                    {formatKw(forecastTotals.distribution)}
                  </td>
                  <td className="px-3 py-2 text-right font-medium">
                    {formatKw(forecastTotals.construction)}
                  </td>
                  <td className="px-3 py-2 text-center text-muted-foreground">
                    {forecastRows.length.toLocaleString("ko-KR")}건
                  </td>
                  <td />
                  <td />
                </tr>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <span className="ml-2 text-muted-foreground">
                      {item.demand_month} · {item.module_width_mm}×{item.module_height_mm} ·{" "}
                      {item.spec_wp}W · {mw(item.required_kw)}
                    </span>
                  </div>
          ...
          <div className="space-y-3 py-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    ...
                    const nextOption = moduleOptions.find(
                      (option) => option.companyId === nextCompanyId,
                    )
                    setForm((p) => ({
                      ...p,
                      companyId: nextCompanyId,
                      moduleKey: nextOption?.key ?? "",
                    }))
                  }}
                >
                  <SelectTrigger>
                    <SelectText text={selectedCompanyName} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  type="month"
                  value={form.demandMonth}
                  onChange={(e) => setForm((p) => ({ ...p, demandMonth: e.target.value }))}
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm((p) => ({ ...p, moduleKey: v ?? "" }))}
                >
                  <SelectTrigger>
                    <SelectText text={selectedModuleLabel} />
                  </SelectTrigger>
                  <SelectContent>
                    {moduleOptions
                      .filter(
                        (option) => companyId !== "all" || option.companyId === form.companyId,
                      )
                      .map((option) => (
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm((p) => ({ ...p, manufacturerId: v ?? "any" }))}
                >
                  <SelectTrigger>
                    <SelectText text={selectedManufacturerName} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  type="number"
                  step="0.001"
                  value={form.requiredMw}
                  onChange={(e) => setForm((p) => ({ ...p, requiredMw: e.target.value }))}
                  placeholder="예) 90"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Select
                  value={form.status}
                  onValueChange={(v) =>
                    setForm((p) => ({ ...p, status: v as ModuleDemandForecast["status"] }))
                  }
                >
                  <SelectTrigger>
                    <SelectText text={selectedStatusLabel} />
                  </SelectTrigger>
                  <SelectContent>
                    {(
                      Object.entries(DEMAND_STATUS_LABEL) as [
                        ModuleDemandForecast["status"],
                        string,
                      ][]
                    ).map(([value, label]) => (
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="예) 일정 유동, Tier1 대체 가능"
              />
            </div>
          </div>
          <DialogFooter>
