          ...
          {summary.avg_profit_ratio > 0 && (
            <span
              className={`sf-status-pill ${
                summary.avg_profit_ratio >= 15
                  ? "sf-tone-pos"
                  : summary.avg_profit_ratio >= 5
                    ? "sf-tone-info"
                    : summary.avg_profit_ratio > 0
                      ? "sf-tone-warn"
                      : "sf-tone-neg"
          ...
          <tbody className="divide-y">
            ...
            <tr className="bg-muted/30 font-medium">
              ...
              <td className="px-2 py-1.5 text-right">
                {formatNumber(summary.total_allocated_qty)} EA
              </td>
              <td className="px-2 py-1.5 text-right">—</td>
              <td className="px-2 py-1.5 text-right">
                {formatNumber(summary.total_cost_amount)}원
              </td>
              <td className="px-2 py-1.5 text-right">
                {formatNumber(summary.total_sales_amount)}원
              </td>
              <td
                className={`px-2 py-1.5 text-right ${summary.total_profit_amount >= 0 ? "text-green-700" : "text-red-700"}`}
              >
                {formatNumber(summary.total_profit_amount)}원
              </td>
              <td
                className={`px-2 py-1.5 text-right ${summary.avg_profit_ratio >= 0 ? "text-green-700" : "text-red-700"}`}
              >
...
function renderRow(m: FifoMatch) {
  ...
  return (
    <tr key={m.match_id} className="hover:bg-muted/40">
      ...
      <td className="px-2 py-1.5 text-right">
        {m.allocated_qty != null ? formatNumber(m.allocated_qty) : "—"}
      </td>
      <td className="px-2 py-1.5 text-right">
        {m.ea_unit_cost != null ? formatNumber(m.ea_unit_cost) : "—"}
      </td>
      <td className="px-2 py-1.5 text-right">
        {m.cost_amount != null ? formatNumber(m.cost_amount) : "—"}
      </td>
      <td className="px-2 py-1.5 text-right">
        {m.sales_amount != null ? formatNumber(m.sales_amount) : "—"}
      </td>
