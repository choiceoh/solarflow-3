...
export default function OutboundDetailView({ outboundId, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    const email = partners
      .find((partner) => partner.partner_id === saleCustomerId)
      ?.contact_email?.trim()
  ...
  const handleSaleCustomerChange = (customerId: string) => {
    ...
    const email =
      partners.find((partner) => partner.partner_id === customerId)?.contact_email?.trim() ?? ""
    setSaleTaxInvoiceEmail(email)
  }

  const usageOptions = (Object.entries(USAGE_CATEGORY_LABEL) as [string, string][]).map(
  ...
  return (
    ...
    >
      ...
      <DetailSection title="제품 · 수량 · 창고">
        <DetailFieldGrid cols={4}>
          <DetailField label="품번"><span className="text-xs">{ob.product_code ?? '—'}</span></DetailField>
          <DetailField label="제조사" value={ob.manufacturer_name} />
          <DetailField label="품명" span={2}><span className="text-xs">{ob.product_name ?? '—'}</span></DetailField>
      ...
      <DetailSection title="메모">
        <DetailFieldGrid cols={1}>
          <EditableDetailField
            ...
            value={ob.memo}
            display={
              ob.memo ? <span className="whitespace-pre-wrap break-words">{ob.memo}</span> : null
            }
            fieldKey="memo"
            editType="textarea"
            disabled={isCancelled}
        ...
        <DetailSection title="매출 정보">
          <DetailFieldGrid cols={4}>
            ...
            <EditableDetailField
              ...
              display={
                ob.sale.unit_price_wp ? `${formatNumber(ob.sale.unit_price_wp)}원/Wp` : undefined
              }
            ...
            <EditableDetailField
              ...
              display={
                ob.sale.unit_price_ea ? `${formatNumber(ob.sale.unit_price_ea)}원` : undefined
              }
            ...
            <DetailField
              ...
              value={
                ob.sale.erp_closed
                  ? `마감 (${formatDate(ob.sale.erp_closed_date ?? "")})`
                  : "미마감"
              }
            />
            <EditableDetailField
            ...
            <EditableDetailField
              ...
              display={
                ob.sale.memo ? (
                  <span className="whitespace-pre-wrap break-words">{ob.sale.memo}</span>
                ) : null
              }
              fieldKey="memo"
              editType="textarea"
          ...
          actions={
            !isCancelled && (
              <Button
                ...
                onClick={() => setSaleFormOpen((open) => !open)}
              >
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  className="gap-1.5"
                  disabled={saleCreating}
                  onClick={createSaleFromOutbound}
                >
                  {saleCreating ? (
