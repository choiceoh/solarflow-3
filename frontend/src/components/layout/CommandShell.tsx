...
function routeMeta(pathname: string, search: string) {
  ...
  if (pathname === "/data" || pathname.startsWith("/data/"))
    return { title: "마스터 관리", breadcrumb: "기준정보 / 운영 기준" }
...
function sumAlertCounts(alerts: AlertItem[], types: string[]) {
  ...
  return alerts.filter((alert) => set.has(alert.type)).reduce((sum, alert) => sum + alert.count, 0)
}
  ...
  const activeTab = useMemo(
    () =>
      tabsConfig && activeTabKey
        ? (tabsConfig.tabs.find((t) => t.key === activeTabKey) ?? null)
        : null,
    [tabsConfig, activeTabKey],
  )
      ...
      <aside className="sf-sidebar" aria-label="주요 메뉴">
        <div className="sf-sidebar-logo">
          ...
          <Link to="/inventory" className="sf-sidebar-logo-text min-w-0">
            ...
            <div className="sf-mono mt-1 text-[9.5px] font-semibold text-[var(--sf-solar)]">
              v3.0 · TOPSOLAR
            </div>
          </Link>
          <button
        ...
        <nav
          className="sf-sidebar-nav"
          aria-label="주요 메뉴 목록"
          data-onboarding-step="self-demo.sidebar.intro"
        >
        ...
        <div className="sf-sidebar-user">
          ...
          <div className="sf-sidebar-user-info min-w-0 flex-1">
            <div className="truncate text-xs font-bold">
              {user?.name || user?.email || "SolarFlow"}
            </div>
            <div className="sf-mono mt-0.5 truncate text-[9.5px] text-[var(--sf-dark-ink-3)]">
              {roleLabel}
            </div>
          </div>
          <button
      ...
      <section className="sf-main-shell">
        <header className="sf-topbar">
          <div className="sf-topbar-title">
            <h1>{meta.title}</h1>
            <div className="sf-topbar-subtitle">
              {meta.breadcrumb} · 계산기준{" "}
