...
function summarizeLines(lines: POLineItem[]): LineSummary {
  if (lines.length === 0)
    return { products: "—", specs: "—", avgCentsPerWp: 0, totalMw: 0, totalUsd: 0 }
  const names = [
    ...new Set(
      lines.map((l) => l.products?.product_code?.split("|")[0]?.trim() ?? l.product_name ?? "—"),
    ),
  ]
  const specs = [
    ...new Set(
  ...
  const totalWp = lines.reduce(
    (s, l) => s + (l.quantity ?? 0) * ((l.products?.spec_wp ?? l.spec_wp ?? 0) / 1000),
    0,
  )
  return {
    products: names.join(", "),
...
/* ─────────────────────────────────────────────
   T/T 행 (펼침 내부 테이블용)
   ───────────────────────────────────────────── */
function TTRow({ idx, tt }: { idx: number; tt: TTRemittance }) {
  ...
  return (
    <tr className="border-t hover:bg-muted/20">
      ...
      <td className="px-3 py-2 text-center">
        <span className={`sf-status-pill ${isDone ? "sf-tone-pos" : "sf-tone-warn"}`}>
          {isDone ? "완료" : "예정"}
        </span>
      </td>
  ...
  if (leafPOs.length === 0) {
    return (
      <div className="rounded-md border bg-muted/30 p-6 text-center text-sm text-muted-foreground">
        계약금 있는 PO가 없습니다 — PO 등록 시 결제조건에서 <strong>계약금 있음</strong>을
        선택하세요
      </div>
    )
  }

  const depositTotals = leafPOs.reduce(
    (acc, po) => {
      ...
      const paidUsd = chainTTs
        .filter((t) => t.status === "completed")
        .reduce((s, t) => s + t.amount_usd, 0)
      const plannedUsd = chainTTs
                  ...
                  >
                    ...
                    <td className="p-3 align-top">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {hasChain && <span className="sf-status-pill sf-tone-warn">변경계약</span>}
                      </div>
                    ...
                    <td className="p-3 text-right align-top min-w-[190px]">
                      ...
                      <div className="mt-1.5 space-y-0.5">
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <span
                            className={`text-[11px] tabular-nums font-mono font-semibold ${
                        ...
                        <div className="space-y-4">
                          {hasChain ? (
                            ...
                            chain.map((chainPo, chainIdx) => {
                              ...
                              const poTTs = tts
                                .filter((t) => t.po_id === chainPo.po_id)
                                .sort((a, b) =>
                                  (a.remit_date ?? "").localeCompare(b.remit_date ?? ""),
                                )
                              const sectionPaid = poTTs
                                .filter((t) => t.status === "completed")
                                .reduce((s, t) => s + t.amount_usd, 0)
                              ...
                              const prevTTCount = chain
                                .slice(0, chainIdx)
                                .reduce(
                                  (s, c) => s + tts.filter((t) => t.po_id === c.po_id).length,
                                  0,
                                )
                                  ...
                                  <div className="flex items-center gap-2 mb-1.5">
                                    <span
                                      className={`sf-status-pill ${
                                        isLeaf ? "sf-tone-info" : "sf-tone-muted"
                                      }`}
                                    >
                                      {isLeaf
                                    ...
                                    <span className="text-[10px] text-muted-foreground">
                                      {poTTs.filter((t) => t.status === "completed").length}건 완료
                                      · {formatUSD(sectionPaid)}
                                    </span>
                                  </div>

                                  {/* T/T 테이블 */}
                                  <TTSection tts={poTTs} startIdx={prevTTCount + 1} />
                                </div>
                              )
                            })
                          ) : (
                          ...
                          <div className="rounded-md bg-muted/40 px-4 py-2.5 flex items-center gap-6 text-xs flex-wrap">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span
                                className={`font-mono font-semibold ${isDone ? "text-green-600" : "text-orange-600"}`}
                              >
                                {formatUSD(paidUsd)}
                              </span>
                            </div>
            ...
            <tr className="border-t bg-muted/20">
              ...
              <td className="p-3 text-right font-mono font-medium tabular-nums">
                ...
                <div className="text-[10px] text-muted-foreground">
                  ...
                  {depositTotals.plannedUsd > 0
                    ? ` · 예정 ${formatUSD(depositTotals.plannedUsd)}`
                    : ""}
                </div>
                <div className="text-[10px] text-red-600">
                  잔여 {formatUSD(depositTotals.remainUsd)}
                </div>
              </td>
              <td />
              <td />
...
/* ─────────────────────────────────────────────
   ...
   ───────────────────────────────────────────── */
function TTSection({ tts, startIdx }: { tts: TTRemittance[]; startIdx: number }) {
  const totals = tts.reduce(
    (acc, tt) => {
        ...
        <tfoot>
          <tr className="border-t bg-muted/20">
            ...
            <td className="px-3 py-1.5 text-xs text-muted-foreground">
              {tts.length.toLocaleString("ko-KR")}건
            </td>
