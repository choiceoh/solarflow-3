...
export default function LCCreateDialog({
  open,
  onClose,
  onCreated,
  initialPoId,
  ...
  useEffect(() => {
    ...
    setAmountUsd(numberInputValue(initialAmountUsd))
    setTargetQty(numberInputValue(initialTargetQty))
    setUsanceDays("")
    setUsanceType("")
    setMaturityDate("")
    setMemo("")
  }, [open, initialPoId, initialAmountUsd, initialTargetQty])
  ...
  useEffect(() => {
    ...
    fetchWithAuth<PurchaseOrder[]>(`/api/v1/pos?company_id=${selectedCompanyId}`)
      .then((list) =>
        setPos(list.filter((p) => p.status !== "cancelled" && p.status !== "completed")),
      )
      .catch(() => setPos([]))
      ...
      <DialogContent className="sm:max-w-2xl max-h-[88vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">
            발주(PO)를 선택하면 해당 PO 한도와 통화로 L/C가 묶입니다. 라인 분할 인수는 등록 후 PO
            상세에서 처리합니다.
          </p>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <FormField size="dense" label="유산스 (일)">
            <Input
              type="number"
              value={usanceDays}
              onChange={(e) => setUsanceDays(e.target.value)}
              placeholder="0 = AT SIGHT"
            />
          </FormField>
          <FormField size="dense" label="유산스 유형">
            <Select value={usanceType} onValueChange={(v) => setUsanceType(v ?? "")}>
              <SelectTrigger>
                <SelectValue placeholder="유산스 유형 선택" />
              </SelectTrigger>
              <SelectContent>
