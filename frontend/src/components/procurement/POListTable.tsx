...
function POListTable({
        ...
        items.map(async (po) => {
          try {
            const [lines, rawLcs, tts] = await Promise.all([
              fetchWithAuth<POLineItem[]>(`/api/v1/pos/${po.po_id}/lines`).catch(
                () => [] as POLineItem[],
              ),
              fetchWithAuth<
                Array<
                ...
                >
              >(`/api/v1/lcs?po_id=${po.po_id}`).catch(() => []),
              fetchWithAuth<TTRemittance[]>(`/api/v1/tts?po_id=${po.po_id}`).catch(
                () => [] as TTRemittance[],
              ),
            ])
            // LC 중첩 flatten
            ...
            const totalWp = (lines ?? []).reduce(
              (s, l) => s + (l.quantity ?? 0) * (l.products?.spec_wp ?? l.spec_wp ?? 0),
              0,
            )
            const first = (lines ?? [])[0]
            result[po.po_id] = {
              ...
              extraCount: Math.max(
                0,
                new Set((lines ?? []).map((l) => l.product_id).filter(Boolean)).size - 1,
              ),
              lcs,
            }
          } catch {
  ...
  async function loadBL(poId: string) {
    ...
    try {
      ...
      setBlDetail((prev) => ({
        ...prev,
        [poId]: { loading: false, bls: blList ?? [], shippingMw, inboundMw, blMwMap },
      }))
    } catch {
      setBlDetail((prev) => ({
        ...prev,
        [poId]: { loading: false, bls: [], shippingMw: 0, inboundMw: 0, blMwMap: {} },
      }))
    }
  }
  ...
  const controlled =
    onSort != null
      ...
      : undefined
  const { sorted, headerProps } = useSort<PurchaseOrder>(
    items,
    (po, f) => {
      ...
      <table className="w-full min-w-[900px] text-xs">
        <thead>
          <tr className="bg-muted/50 border-b">
            ...
            <SortableTH
              {...headerProps("contract_type")}
              className="p-3 font-medium text-muted-foreground"
            >
              계약 조건
            </SortableTH>
            <SortableTH {...headerProps("totalUsd")} align="right" className="p-3 font-medium">
              계약금액 / 결제
            </SortableTH>
            <SortableTH
              {...headerProps("lcUsd")}
            ...
            <SortableTH
              {...headerProps("status")}
              align="center"
              className="p-3 font-medium text-muted-foreground w-[80px]"
            >
              상태
            </SortableTH>
                ...
                >
                  ...
                  <td className="p-3 align-top">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {po.parent_po_id && (
                        <StatusPill label="변경계약" tone="warning" className="text-[9px]" />
                      )}
                    </div>
                    {po.contract_date && (
                      <div className="text-[10px] text-muted-foreground font-mono mt-0.5">
                        {formatDate(po.contract_date)}
                      </div>
                    )}
                      ...
                      (() => {
                        ...
                        return p ? (
                          <div className="text-[10px] text-muted-foreground">
                            원계약: {p.po_number ?? p.po_id.slice(0, 8)}
                          </div>
                        ) : null
                      })()}
                  </td>
                  ...
                  <td className="p-3 align-top min-w-[160px]">
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      {a && a.totalMw > 0 && (
                        <span className="text-[10px] font-semibold tabular-nums font-mono">
                  ...
                  <td className="p-3 align-top min-w-[130px]">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <StatusPill
                        label={CONTRACT_TYPE_LABEL[po.contract_type]}
                        tone="neutral"
                        className="px-2"
                      />
                      {po.incoterms && (
                    ...
                    <div className="mt-1 space-y-0.5">
                      {pt.depositPct && (
                        <div className="text-[10px] text-muted-foreground">
                          계약금 <span className="text-blue-600 font-medium">{pt.depositPct}%</span>{" "}
                          T/T
                        </div>
                      )}
                      {pt.lcUsance && (
                        <div className="text-[10px] text-muted-foreground">
                          L/C Usance <span className="font-medium">{pt.lcUsance}일</span>
                        </div>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="p-3 text-right align-top min-w-[160px]">
                    <div className="font-semibold tabular-nums font-mono">
                      {a ? formatUSD(a.totalUsd) : "—"}
                    </div>
                    {ttBudget !== null && (
                      <div className="mt-1 space-y-0.5">
                        <div className="text-[10px] text-muted-foreground">
                          계약금 {pt.depositPct}%:{" "}
                          <span className="font-mono">{formatUSD(ttBudget)}</span>
                        </div>
                        <div className="flex items-center justify-end gap-1">
                          ...
                          />
                          <span
                            className={`text-[10px] tabular-nums font-mono ${ttDone ? "text-green-600" : "text-orange-500"}`}
                          >
                            {formatUSD(ttPaid)}
                            {ttDone ? " ✓" : ""}
                          </span>
                  ...
                  <td className="p-3 text-right align-top min-w-[150px]">
                    {a && (a.lcUsd > 0 || a.totalUsd > 0) ? (
                      <>
                        <div className="font-semibold tabular-nums font-mono text-blue-700">
                          {formatUSD(a.lcUsd)}
                        </div>
                                  ...
                                  <span className="ml-2 text-green-600">
                                    ...
                                    {bl.shippingMw > bl.inboundMw + 0.01 && (
                                      <span className="ml-1.5 text-blue-600">
                                        선적중 {bl.shippingMw.toFixed(2)} MW
                                      </span>
                                    )}
                                  </span>
                                )}
                              ...
                              <tbody>
                                {(() => {
                                  ...
                                  return a.lcs.map((lc, lcIdx) => {
                                    const lcBls = (bl?.bls ?? []).filter(
                                      (b) => b.lc_id === lc.lc_id,
                                    )
                                    const mLabel = moduleLabel(
                                      ...
                                      <Fragment key={lc.lc_id}>
                                          ...
                                          <td className="px-3 py-2">
                                            ...
                                            {lc.target_mw ? (
                                              <div className="text-[10px] text-muted-foreground font-mono">
                                                {lc.target_mw.toFixed(2)} MW
                                              </div>
                                            ) : null}
                                          </td>
                                          <td className="px-3 py-2 text-muted-foreground">
                                            {formatDate(lc.maturity_date ?? "")}
                                          </td>
                                        ...
                                        {lcBls.length > 0 ? (
                                              ...
                                              >
                                                ...
                                                <td className="px-3 py-1.5 text-center">
                                                  <StatusPill
                                                    ...
                                                    colorClassName={
                                                      BL_STATUS_COLOR[b.status] ?? "sf-tone-muted"
                                                    }
                                                  />
                                                </td>
                                              </tr>
                                          ...
                                          <tr className="border-t bg-muted/5">
                                            <td
                                              colSpan={6}
                                              className="pl-8 pr-3 py-1.5 text-[10px] text-muted-foreground italic"
                                            >
                                              입고 미등록
                                            </td>
                                          </tr>
        ...
        <tfoot>
          <tr className="border-t bg-muted/20">
            ...
            <td className="p-3 font-mono font-medium tabular-nums">
              {totals.totalMw > 0 ? `${totals.totalMw.toFixed(2)} MW` : "—"}
            </td>
            <td />
            ...
            <td className="p-3 text-right font-mono font-medium tabular-nums">
              ...
              <div className="text-[10px] text-muted-foreground">
                {totals.lcMw > 0 ? `${totals.lcMw.toFixed(2)} MW` : "—"}
              </div>
            </td>
            <td />
            <td />
