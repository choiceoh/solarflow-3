...
function buildColumns({ currency, manufacturerName }: BuildOpts): ColumnDef<BLLineItem>[] {
  return [
    ...
    { key: 'product_code', label: '품번', hideable: true, className: 'font-mono text-[13px]', cell: (l) => pCode(l), sortAccessor: (l) => pCode(l) },
    { key: 'product_name', label: '품명', hideable: true, className: 'text-[13px]', cell: (l) => pName(l), sortAccessor: (l) => pName(l) },
