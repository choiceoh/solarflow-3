...
기본 워크플로는 **자동 적용 (안전 추정 기본)**. `cron-deploy.sh` 가 webhook/cron 회차에 `scripts/apply_migrations.py` 를 호출해 미적용 파일을 결정 → 적용한다.
...
backend/.venv-ocr/bin/python scripts/apply_migrations.py    # 자동 결정대로 모든 미적용 파일 일괄
