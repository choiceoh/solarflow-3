...
- **입력**: 엑셀 Import 허브 (통합 양식 다운로드 + 업로드 + 서버 검증)
- **출력**: 엑셀 Export (아마란스10 양식, 보고서, 분석 자료)
- **핵심 가치**: 통합 분석 시스템 (재고 현황, 마진 분석, LC 한도, 대시보드, 검색)
- **웹 화면의 역할**: 조회·분석·검색·상태 파악 중심. 운영 데이터 생성은 기본적으로 웹 입력칸을 두지 않는다.
- **수정 허용 범위**: 데이터 간 연결 지정, 상태 전환, 수금 매칭, 서류 첨부처럼 기존 데이터의 관계와 진행 상태를 정리하는 작업으로 제한한다.
...
| product_family_code | TEXT | | 품번 상위 제품군 코드. 같은 제조사·시리즈·라인·외형 규격 묶음 |
| product_variant_kind | VARCHAR(30) | | output_bin/bom_variant/cert_variant/label_variant/packaging_variant/mixed/other |
| bom_revision | TEXT | | 동일 출력·동일 제품군에서 BOM 차이로 갈라진 revision |
| substitution_group_code | TEXT | | 출고·영업 검토용 수동 대체그룹. 자동 대체 확정값 아님 |
...
※ 품번은 PO/B/L/재고/원가/매출의 거래 SKU로 유지한다. 제품군은 같은 생산 라인·외형 규격을 묶는 분석/검토 축이며, `product_aliases`처럼 품번을 canonical로 합치는 용도로 쓰지 않는다.
※ 출력만 다른 경우는 같은 `product_family_code` + `product_variant_kind='output_bin'`, 동일 출력이지만 BOM이 다른 경우는 같은 `product_family_code` + `product_variant_kind='bom_variant'` + `bom_revision`으로 구분한다.
...
| status | VARCHAR(20) | ✅ | draft/contracted/in_progress/completed/cancelled |
| memo | TEXT | | |
| parent_po_id | UUID(FK) | | 변경계약의 원계약 PO |
...
| unit_price_usd | DECIMAL(10,6) | | 패널당 USD 단가 |
...
- 진행률 바 + 잔여량 표시
- PO 라인별 계약량 / LC개설량 / 선적완료 / 입고완료 / 잔여량 / 입고율 표시

#### 변경계약 작성
- PO 상세에서 "변경계약 작성"을 누르면 현재 PO를 `parent_po_id`로 연결한 새 PO 초안을 작성한다.
...
3. 사용자가 미수금 행을 선택하고 행별 매칭금액을 입력 → 하단에 "선택 합계" 실시간 변동
4. 매칭금액 합계가 입금 가능액을 넘지 않으면 [매칭 확정]
5. 차액: 선수금 / 다음 정산 이월 / 환불·정산 검토 중 선택 후 확정
6. 출고/판매 화면의 [수금완료]는 해당 매출의 미수 잔액만큼 수금 전표와 매칭 전표를 자동 생성한다. 수금 관리 목록은 기본적으로 완전 매칭 전표를 숨기고 미수/부분매칭 항목만 보여준다.
7. 판매 목록은 수금 상태(open/unpaid/partial/paid)로 필터할 수 있다. `수금 미완료` 업무 큐에서는 처리 가능한 미수 행을 자동 선택하고 [현재 목록] 또는 [필터 전체] 수금완료로 여러 매출의 수금 전표와 매칭 전표를 한 번에 생성한다.
8. 판매/수금 일괄 처리는 실행 전 대상 건수, 금액 합계, 제외 건수와 제외 사유를 사전 검토 다이얼로그로 확인한다. 표의 선택 불가 행은 빈 체크박스 대신 `제외` 표시와 사유 tooltip을 보여 사용자가 어떤 데이터를 먼저 보정해야 하는지 알 수 있게 한다(D-20260512-143605).
...
#### WMS 자동화 축 (D-154)
- 출고가 `active` 로 생성되면 WMS 피킹 명세를 자동 생성한다. 같은 출고의 기존 명세가 있으면 중복 생성하지 않는다.
- 피킹 위치는 최신 수불(`inventory_movements.location_code` + `ending_qty`)을 우선 사용하고, 부족하면 위치 배정(`inventory_allocations.location_id`)을 fallback 으로 사용한다. 위치 자료가 없으면 출고 작업 큐 누락 방지를 위해 무위치 라인으로 생성한다.
- B/L 이 `completed` 또는 `erp_done` 으로 전환되면 BL 라인별 입고 검수 로그(`receiving_logs`)를 자동 생성한다. 그룹내 매입 요청은 BARO 입고확인 시 `source_type='intercompany'` 검수 로그를 자동 생성한다. 검수자는 이후 위치, 실수량, 차이 사유, 사진, 메모를 보정한다.
- 재고실사 세션은 위치별 시스템 재고를 자동 seed 한 뒤 실측 입력으로 넘어간다. seed 는 최신 위치 수불을 우선하고 위치 배정 재고 합산을 fallback 으로 사용한다.
- 실사 차이의 실제 재고 보정은 결재/감사 기준이 필요하므로 별도 단계에서 처리한다.
...
- 기본 계산은 품번별(product_id)로 각각 생성한다. 제품군(product_family_code)은 재고/가격/매출 분석의 보조 집계 축으로만 사용하고, 수주 충당·원가·회계 확정 계산은 자동으로 합산하지 않는다.
- 법인 전체 합산 + 법인별 분리 가능
...
- `/sales-analysis`는 공통 필터(기간, 거래처, 원가 기준, 제조사)를 유지하고, 화면 본문을 `요약`, `매출이익`, `제조사별`, `거래처별`, `미수·수금`, `대사` 탭으로 나눈다. 요약은 경영 KPI와 우선순위, 매출이익은 월별 매출·브리지·대체원가·품목별 이익, 제조사별은 제조사 기여도, 거래처별은 거래처 청구/미수, 미수·수금은 연체/미발행 후보, 대사는 원장·계산서·원가·수금 후보 행을 담당한다(D-20260512-132713).
...
#### KPI 표시 설정
KPI 타일은 사용자별로 화면/탭 단위 활성 항목을 설정할 수 있다. 설정값은 `user_profiles.preferences.kpi_hidden`에 `{ scope: [kpi_key...] }` 형태로 저장하며, 최소 1개 KPI는 항상 표시한다. 사이트 공통 KPI 정의는 코드/설계 정본을 유지하고, 사용자 설정은 표시 여부만 제어한다. KPI 표시 메뉴는 화면 우상단 액션 영역의 엑셀 입력 버튼 옆에 배치해 필터/테이블 공간을 침범하지 않게 한다.
...
3. SolarFlow에 업로드 → 서버 검증 → DB 저장
   ...
   에러: "3행: 제조사가 존재하지 않습니다. alias 후보: ..." 행별 에러 반환
4. SolarFlow에서 아마란스10 양식 내보내기 → ERP 업로드
5. 웹 화면은 조회/분석을 기본으로 하며, 직접 생성 폼은 운영 기본 경로에서 제거
   수정은 데이터 연결 지정, 상태 전환, 수금 매칭, 서류 첨부처럼 관계·진행 상태 보정에 한정
```

양식 종류: 입고, 출고, 매출, 면장/원가, 부대비용, 수주, 수금
통합 양식: 위 양식을 한 파일의 여러 시트로 내려받아 같은 입력 허브에서 업로드
양식 버전: 다운로드 파일의 숨김 메타 시트 `_SolarFlowMeta`에 `template_version`, `template_kind`, `template_types`를 기록하고, 업로드 시 지원 버전인지 확인한다.
리허설: PO/LC/T/T 정상·경고·오류 행이 섞인 샘플팩으로 미리보기, alias 후보, 오류 다운로드, 등록 전 영향 요약을 검증한다.
등록 전 요약: 통합 업로드는 확정 전 PO 건수/라인/수량, LC 금액/MW, T/T USD/KRW, 경고/오류 건수를 먼저 표시한다.
미리보기 수정: 통합 업로드 미리보기에서 에러/경고 셀을 직접 보정하고 즉시 재검증할 수 있다. 원본 엑셀 파일은 덮어쓰지 않고, 확정 전 화면 데이터에만 반영한다.
오류 해결 안내: 미리보기 에러/경고 목록은 해당 행으로 이동하는 기능과 함께 필수값 누락, 마스터 미등록, 양수·날짜·허용값·중복 오류별 보정 안내를 표시한다(D-20260512-143605).
보정본 다운로드: 화면에서 보정한 현재 preview 데이터는 다시 업로드 가능한 통합 엑셀로 내려받을 수 있다. 보정본에는 `_SolarFlowMeta`, 작성안내, 통합코드표, 업무별 입력 시트와 함께 `검토요약` 시트를 넣어 수정 전/후 값과 남은 오류·경고를 함께 남긴다.
업로드 이력: Import Hub는 브라우저별 최근 업로드 미리보기/등록 결과 요약을 보관해 실패·경고 파일을 다시 찾을 수 있게 한다. 이력은 작업 보조용이며 감사 로그 정본이 아니다.
처리 대기 통합 센터: Import Hub 상단에는 업로드 검토, 매출 미등록, 계산서 미발행, ERP 미마감, 수금 미완료 카드를 표시한다. 카드는 기존 대시보드/판매 요약 API를 사용하고 Orders 화면의 필터 URL로 연결한다.
등록 결과 후속 액션: 통합 등록 결과 다이얼로그는 성공한 섹션에 따라 매출 미등록, 계산서 미발행, 수금 미완료, 수금매칭, 구매 계약, B/L 입고, 면장/원가 확인 화면으로 바로 이동하는 다음 행동 버튼을 제공한다(D-20260512-143605).
...
PO 상세 변경이력 탭은 `audit_logs.old_data/new_data`를 한국어 필드명 기준 diff로 표시한다.
...
Import Hub에는 `문서 OCR` 탭을 두고, 일반 문서는 원문 텍스트를 확인·복사하는 흐름으로 처리한다. 수입필증/면장은 `document_type=customs_declaration`으로 구조화 후보를 함께 표시하되, 사용자가 확인하거나 별도 import 흐름으로 넘기기 전까지 DB에 자동 저장하지 않는다.
...
```
[구매 흐름]
PO 등록(엑셀) → T/T 계약금(엑셀/연결 상태 확인) → LC 개설(엑셀/한도 차감)
...
또는 출고/판매 [수금완료] → 미수 잔액 수금 전표 + 매칭 자동 생성
...
[웹 핵심 흐름]
조회/검색/분석 → 연결 상태 확인 → 필요한 관계 지정/상태 전환/매칭
```
...
SolarFlow의 화면은 같은 데이터와 API를 공유하되, 사용 목적에 따라 표면을 분리한다.

- **조회자용 화면**: KPI, 분석, 추세, 위험 신호를 먼저 보여준다. 경영진·관리자가 현재 상태를 빠르게 파악하는 것이 목적이며, 데이터 생성 CTA는 기본 표면에 두지 않는다.
- **입력자용 화면**: 처리 큐, 엑셀 업로드, 검증 오류, 보정, 일괄 실행 전 확인을 먼저 보여준다. 실무자가 반복 입력과 오류 해결을 빠르게 끝내는 것이 목적이며, 장식성 KPI보다 대상 목록·필터·실행 버튼을 우선한다.
- **구현 기준**: 역할별로 앱을 포크하지 않고, 라우트와 컴포넌트는 공유한다. `/import`, `/orders`, `/procurement` 같은 운영 화면은 입력자/처리자 흐름을 기본으로 정리하고, `/sales-analysis`, `/insights` 같은 분석 화면은 조회자 흐름을 유지한다.
- **Orders 처리 모드**: `/orders`는 매출 미등록, 계산서 미발행, ERP 미마감, 수금 미완료 큐를 본문 상단 작업 시작점으로 둔다. 큐가 선택된 동안은 KPI보다 대상 목록과 실행 패널을 먼저 보이게 하며, 우측 분석 보조 정보는 유지하되 처리 대기 버튼을 중복 노출하지 않는다.
...
→ [PO 상세]: 기본정보 + 라인아이템 + LC현황 + TT이력 + 입고현황(라인별 진행률) + 변경이력(diff)
→ [변경계약 작성] → 원계약 연결 PO 초안 생성 → 계약 확정
```
