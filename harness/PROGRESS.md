...
- 차트 프리셋 추가
  ...
  - 우리 거래가
  - FOB · DDP · 입찰
- 차트/테이블 라인 선택 시 우측 `선택 라인` 패널에 최근값과 직전 대비 표시
- CMM 추세, 우리 구매가 대비, CPIA floor 근접도를 이용한 1개월/3개월 전망과 구매 전략 카드 추가
- 가격예측 표/차트 필터를 source 외에 basis, market_region, technology, quarter/period까지 확장
- 표를 18행 단위 페이지로 분리하고 현재 범위/전체 건수/이전·다음 이동 추가
- 필터 적용 결과 CSV 내보내기와 AI 수집 중복 실행 방지 추가
- AI 수집 로그에 source별 상태 badge와 warning 건수 표시 추가
- 최신 main의 AI 관측값 선택 삭제 기능을 페이지네이션 표에 통합
- 백엔드 AI 저장 정책 강화
  - 허용된 source_key, metric_key, market_region, basis, currency만 저장
  - InfoLink cell/wafer는 프롬프트 지시와 별개로 서버에서 차단
  - `TestValidateBenchmarkCatalogPolicy` 추가
- Tier-1 ASP 제거(D-145)
...
- `cd frontend && npm run lint` 종료코드 0 — 기존 baseline 경고 출력
- `git diff --check` 성공
- `graphify update .` 성공
...
- `price_benchmarks`, `price_benchmark_runs` 마이그레이션 추가
  - OPIS CMM/forward/DDP, InfoLink, TrendForce, PVinsights, 중국 국영 입찰, CPIA floor를 공통 시계열 구조로 저장
