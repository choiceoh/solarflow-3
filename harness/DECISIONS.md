...
## D-105: 목업 로그인은 실데이터 API 앞에서 차단
- **결정**: PR19 디자인 검토용으로 개발/운영 모두에서 목업 로그인을 제공한다. 이 모드에서는 Supabase 로그인이나 Go/PostgREST/Rust API를 호출하지 않고, 프론트엔드의 `fetchWithAuth` 앞단에서 목업 응답을 즉시 반환한다.
- **이유**: 실제 계정/DB 준비가 막히면 디자인 이관 품질을 눈으로 비교할 수 없다. 단순 인증 우회만 만들면 보호 라우트 진입 뒤 실제 자료 DB를 다시 호출하므로, 로그인 우회와 API 목업 계층을 함께 둬야 실데이터와 완전히 분리된다.
- **운영 기준**: 기본값은 목업 버튼 노출이며, `VITE_ENABLE_DEV_MOCK_LOGIN=false`를 명시한 빌드에서만 숨긴다. 목업 세션이 켜진 동안 `/api/v1/*`, 계산 API, OCR, 파일 export 요청은 모두 프론트 목업 응답으로 처리하고, 운영 로그인/권한 로직은 그대로 유지한다.
- **날짜**: 2026-05-01

## D-106: PR19 재현도는 공통 부품과 256px 레일 구조를 기준으로 맞춘다
- **결정**: PR19 화면 이관은 개별 화면에서 비슷한 카드만 배치하는 방식이 아니라, 목업의 공통 `TileB`, `CardB`, `RailBlock`, 212px 사이드바, 56px 헤더, 256px 우측 레일 구조를 실제 프론트 기준선으로 삼는다. Shell 헤더가 이미 화면 제목을 갖고 있으므로 본문에서 같은 제목을 반복하지 않는다.
- **이유**: 이전 1차 이관은 기능 연결은 되었지만 KPI 간격, 카드 헤더 높이, 우측 워치 레일 시작점, 필터 위치가 목업과 달라 화면이 “PR19 느낌”에 머물렀다. 공통 부품과 레일 구조를 먼저 맞춰야 이후 실제 기능을 붙여도 디자인이 계속 같은 시스템 안에서 진화한다.
- **운영 기준**: 재고, 면장/원가처럼 우측 레일이 있는 화면은 본문 내부에 `minmax(0,1fr) 256px` 구조를 둔다. 대시보드는 KPI와 중앙 작업 영역을 본문 컬럼에 두고, 워치 레일은 헤더 바로 아래부터 독립 레일로 시작한다. 탭·필터·주요 액션은 가능하면 `CardB` 헤더 우측에 모아 표 위 공간을 줄인다. 모바일/좁은 폭에서는 레일을 아래로 접고 표 구조는 유지한다.
- **날짜**: 2026-05-01

## D-107: 상단 검색창은 헤더 전체 축을 기준으로 고정
- **결정**: PR19 상단 검색창은 페이지 제목이나 우측 액션 폭에 따라 움직이지 않도록, 헤더를 좌측 정보/중앙 검색/우측 액션 3컬럼 그리드로 고정한다.
...
## D-115: 운영 데이터 생성은 Excel Import Hub로 단일화, 웹은 조회·분석 중심
- **결정**: 운영 화면에서 `새로 등록`, `수동 등록`, 직접 생성 폼을 여는 기본 CTA를 제거하고, 생성 경로는 `/import` Excel Import Hub로 모은다. 상단 전역 액션도 `빠른 등록` 드롭다운 대신 `엑셀 입력` 진입점으로 바꾼다.
- **웹 화면의 역할**: 목록·상세·검색·분석·이력 확인을 기본 역할로 둔다. 수정 기능은 데이터 간 연결 지정, 상태 전환, 수금 매칭, 서류 첨부처럼 이미 존재하는 데이터의 관계와 진행 상태를 보정하는 작업에 한정한다.
- **이유**: 화면마다 입력칸이 열리면 사용자가 "웹에서 직접 등록"과 "엑셀 양식 업로드" 중 어느 경로가 정식 운영 경로인지 혼동한다. SolarFlow의 본질은 엑셀 원본을 안전장치로 유지하면서 검증된 데이터를 모아 보는 분석 플랫폼이므로, 입력 UX를 엑셀 허브로 단일화해야 한다.
- **운영 기준**:
  - Excel Import Hub는 통합 양식 다운로드와 업무별 양식 다운로드/업로드를 제공한다.
  - 기존 CRUD API와 폼 컴포넌트는 import 처리, 연결/상태 보정, admin 유지보수 경로를 위해 즉시 삭제하지 않는다. 다만 운영 메뉴에서 직접 생성 진입점은 숨긴다.
...
## D-150: 매출 분석은 원인 분해와 우선순위 큐까지 한 화면에서 보여준다

- **결정**: `/sales-analysis`는 KPI 숫자만 나열하지 않고, 이익 원인 분해(저마진 품목·원가 공백), 거래처 위험 우선순위(미수·연체·저마진 복합), 제조사별 기여도, 원장 대사 체크를 같은 화면에 노출한다.
- **이유**: 경영진이 필요한 판단은 "이익률이 몇 %"에서 끝나지 않는다. 왜 떨어졌는지, 어떤 품목·제조사·거래처가 우선인지, 매출원장과 이익엔진 숫자가 같은 범위를 보고 있는지를 즉시 확인해야 한다.
- **운영 기준**:
  - 거래처 필터를 선택하면 매출 원장, 거래처 분석뿐 아니라 `margin-analysis`에도 `customer_id`를 전달해 이익 KPI 범위를 일치시킨다.
  - 이익 누수는 포트폴리오 평균 이익률 대비 낮은 품목의 매출 기여액과 원가 미연결 매출을 분리해서 표시한다.
  - 원장 대사 체크는 매출원장 공급가와 이익엔진 공급가 차이, 세금계산서 미발행, 원가 미연결, 수금 미회수를 각각 정상/주의/위험으로 구분한다.
- **검증**:
...
## D-152: `/purchase-history` 선택 체인은 감사 렌즈와 점검 포인트를 함께 보여준다

- **결정**: `/purchase-history`는 기존 통합 타임라인 위에 선택 체인 단위 감사 렌즈를 추가한다. 감사 렌즈는 최근 1년 `audit_logs`에서 PO 변경 건수, 변경 필드 수, 변경 주체, 최근 감사일, 연결 원천 행 수를 요약하고, 기존 PO/단가/L/C/B/L/T/T 이벤트를 해석해 점검 포인트를 산출한다.
- **점검 기준**:
  - `purchase_orders` 감사 변경/취소는 변경 전후 필드를 구조화해 타임라인 이벤트에 표시한다.
  - PO 미연결 단가, 미결제 L/C, 예정 T/T, 진행 중 B/L, 금융 이력 부재, B/L 이력 부재, 90일 이상 무변동 계약은 read-only 점검 포인트로 노출한다.
  - 점검 포인트는 업무 차단/자동 판정이 아니라 운영자가 원본 화면으로 이동해 확인할 힌트다. D-113의 read-only 원칙을 유지한다.
- **이유**: 1차 구매이력 UX는 "무슨 일이 언제 있었는가"를 해결했다. 운영에서는 한 단계 더 나아가 "이 계약에서 확인해야 할 찜찜한 부분이 무엇인가"와 "누가 어떤 필드를 바꿨는가"가 필요하다. 서버 감사 모델을 새로 만들기 전에도 기존 이벤트와 감사 로그만으로 충분한 1차 감사 렌즈를 제공할 수 있다.
- **운영 기준**:
  - 이벤트 검색은 제목/세부/원천 테이블/원천 ID/변경 주체/변경 전후 값을 대상으로 한다.
...
## D-20260512-112516: Import Hub는 오류 수정·업로드 이력·업무 큐·문서 OCR을 한 흐름에 둔다

- **결정**: Import Hub에 통합 업로드 미리보기 셀 수정, 수정내역 추적, 보정본 엑셀 다운로드, 브라우저별 최근 업로드 이력, 처리 대기 통합 카드, 문서 OCR 탭을 함께 둔다. Orders 화면은 Import Hub 카드에서 넘어오는 매출 미등록, 계산서 미발행, ERP 미마감, 수금 미완료 필터 URL을 해석한다.
- **운영 기준**:
  - 미리보기 셀 수정은 메모리상의 업로드 preview 데이터만 바꾸고 원본 엑셀 파일은 덮어쓰지 않는다.
  - 셀 수정 후에는 PO/LC/T/T/원가 선언/부대비용 검증을 다시 수행하고, 최종 등록은 기존 import 확정 경로를 그대로 사용한다.
  - 수정한 셀은 행/필드/수정 전/수정 후 값을 추적하고, 보정본 엑셀의 `검토요약` 시트에 남긴다.
  - 보정본 엑셀은 `_SolarFlowMeta`, 작성안내, 통합코드표, 업무별 입력 시트를 포함해 다시 Import Hub에 업로드할 수 있어야 한다. 다만 이 파일은 작업 보조 산출물이며 DB 감사 로그를 대체하지 않는다.
  - 업로드 이력은 localStorage의 요약 이력으로만 보관하며, 감사 로그나 장부 정본으로 쓰지 않는다.
  - 처리 대기 카드는 기존 dashboard/summary API를 읽고 `/orders?tab=...` 필터 링크로 이동한다.
  - 문서 OCR 탭은 기존 `POST /api/v1/ocr/extract`를 사용한다. 일반 문서는 원문 텍스트, 수입필증/면장은 구조화 후보를 표시하되 자동 DB 저장은 하지 않는다.
- **이유**: 업로드 실패 파일을 수정·재업로드하는 반복 시간을 줄이고, 입력자가 다음 처리 대상을 Import Hub에서 바로 발견하게 하기 위해서다. OCR도 별도 화면을 찾지 않고 입력 허브에서 먼저 읽어 볼 수 있어야 한다.
- **검증**:
  - `cd frontend && npx --yes bun@1.3.13 run build`
  - `cd frontend && npx --yes bun@1.3.13 run lint`
  - `git diff --check`
  - `graphify update .`
- **날짜**: 2026-05-12 11:25:16 KST

## D-20260512-132713: 매출 분석 화면은 관점별 탭으로 세분화한다

- **결정**: `/sales-analysis`는 공통 필터(기간, 거래처, 원가 기준, 제조사)를 상단에 유지하고, 본문을 `요약`, `매출이익`, `제조사별`, `거래처별`, `미수·수금`, `대사` 탭으로 나눈다.
- **운영 기준**:
  - `요약`: 경영 KPI, 이익 원인 분해, 거래처 위험 우선순위.
  - `매출이익`: 월별 매출, 이익률 변동 브리지, 대체원가 기준 마진, 품목별 이익.
  - `제조사별`: 제조사별 매출 비중, 이익률, 원가 공백, 출고 용량.
  - `거래처별`: 거래처별 청구액, 미수, 이익률.
  - `미수·수금`: 미수 위험 거래처, 수금액, 미수금, 계산서 미발행 후보.
  - `대사`: 매출원장↔이익엔진 차이, 계산서 미발행, 원가 미연결, 수금 미회수 후보 행.
- **이유**: 기존 한 화면 스크롤은 제조사·거래처·미수·이익·대사 판단이 한 덩어리로 보여, 사용자가 “무엇을 판단하는 화면인지”를 매번 다시 해석해야 했다. 탭을 나누면 기존 D-150의 원인 분해 의도는 유지하면서도 경영 보고, 이익 점검, 미수 처리, 제조사/거래처 비교를 각자 자연스러운 흐름으로 볼 수 있다.
- **검증**:
  - frontend build 로 TypeScript와 Vite 번들 생성을 확인한다.
  - 계산 API와 Rust 엔진 계약은 변경하지 않는다.
- **날짜**: 2026-05-12 13:27:13 KST

## D-20260512-143605: 입력·일괄처리는 다음 행동과 사전검토를 먼저 보여준다

- **결정**: Import Hub와 판매 일괄 처리 흐름에 사용자가 다음 조치를 바로 판단할 수 있는 보조 UX를 추가한다.
  - 통합 등록 결과 다이얼로그는 성공한 섹션을 기준으로 매출 미등록, 계산서 미발행, 수금 미완료, 수금매칭, 구매 계약, B/L 입고, 면장/원가 확인 화면으로 이동하는 다음 행동 버튼을 제공한다.
  - 통합 업로드 미리보기의 에러/경고 목록은 행 이동뿐 아니라 필수값, 마스터 미등록, 양수, 날짜, 중복, 허용값 오류별 해결 안내를 함께 표시한다.
  - 매출 자동 생성, 계산서 일괄 처리, ERP 일괄 마감, 현재 목록/필터 전체 수금완료는 실행 전 대상 건수, 금액 합계, 처리일, 제외 건수와 제외 사유를 확인한다.
  - 판매 표의 일괄 처리 선택 칸은 처리 불가 행을 빈칸으로 두지 않고 `제외` 표시와 사유 tooltip을 보여준다.
- **이유**: 사용성 개선은 새 입력 경로를 늘리는 것보다 기존 운영 흐름의 다음 클릭을 줄이는 쪽이 SolarFlow 원칙에 맞다. Import 결과, 오류 목록, 일괄 처리 버튼은 모두 장부 변경 또는 후속 처리의 시작점이므로 사용자가 대상·금액·제외 사유를 확인하고 바로 다음 화면으로 이동할 수 있어야 한다.
- **운영 기준**:
  - 다음 행동 버튼은 화면 이동만 수행하며 DB에 자동 조치를 하지 않는다.
  - 오류 해결 안내는 자동 확정이 아니라 사용자가 셀 수정, 원본 엑셀 보정, 마스터/alias 정리를 선택하도록 돕는 설명 레이어다.
  - 일괄 처리 사전검토 후에도 서버 API는 기존 건별 검증 경로를 그대로 사용해 과매칭, 취소 행, 이미 처리된 행을 다시 차단한다.
- **검증**:
  - `cd frontend && npx --yes bun@1.3.13 install --frozen-lockfile`
  - `cd frontend && npx --yes bun@1.3.13 run build`
  - `cd frontend && npx --yes bun@1.3.13 run lint`
  - `git diff --check`
  - `graphify update .`
