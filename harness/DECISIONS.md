...
## D-105: 개발용 목업 로그인은 실데이터 API 앞에서 차단
- **결정**: PR19 디자인 검토용으로 개발 전용 목업 로그인을 제공한다. 이 모드에서는 Supabase 로그인이나 Go/PostgREST/Rust API를 호출하지 않고, 프론트엔드의 `fetchWithAuth` 앞단에서 목업 응답을 즉시 반환한다.
...
- **운영 기준**: 개발 서버(`import.meta.env.DEV`), 로컬 브라우저(`localhost`/`127.0.0.1`), 또는 `VITE_ENABLE_DEV_MOCK_LOGIN=true` 빌드에서만 목업 버튼을 노출한다. 목업 세션이 켜진 동안 `/api/v1/*`, 계산 API, OCR, 파일 export 요청은 모두 프론트 목업 응답으로 처리하고, 운영 로그인/권한 로직은 그대로 유지한다.
- **날짜**: 2026-05-01
