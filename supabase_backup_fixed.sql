...
CREATE FUNCTION public.get_user_role() RETURNS public.user_role
    ...
    AS $$
  SELECT role FROM user_profiles WHERE id = current_setting('request.jwt.claims', true)::json->>'sub'
$$;
...
CREATE TABLE public.bl_line_items (
    bl_line_id uuid DEFAULT gen_random_uuid() NOT NULL,
    bl_id uuid NOT NULL,
...
CREATE TABLE public.bl_shipments (
    bl_id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.lc_records (
    lc_id uuid DEFAULT gen_random_uuid() NOT NULL,
    po_id uuid NOT NULL,
...
CREATE TABLE public.manufacturers (
    manufacturer_id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.price_histories (
    price_history_id uuid DEFAULT gen_random_uuid() NOT NULL,
    product_id uuid,
    manufacturer_id uuid NOT NULL,
    change_date date NOT NULL,
...
CREATE TABLE public.tt_remittances (
    tt_id uuid DEFAULT gen_random_uuid() NOT NULL,
    po_id uuid NOT NULL,
    remit_date date,
    amount_usd numeric(15,2) NOT NULL,
