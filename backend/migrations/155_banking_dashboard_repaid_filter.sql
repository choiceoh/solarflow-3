-- M155: banking_dashboard RPC "사용중" 필터 수정
--
-- 배경:
--   기존 used_per_bank / lc_open_filtered CTE 가
--     WHERE status <> 'settled' AND repaid IS NOT TRUE
--   였는데, usance LC 는 status='settled' (개설은행→공급자 결제완료) 가 돼도
--   우리(수입자)→개설은행 상환 (repaid=true) 전까지 한도를 점유함.
--   결과적으로 운영 DB 의 LC 63건 중 62건 (settled, repaid=false) 이 전부
--   "사용중" 집계에서 빠지고, status='cancelled' 1건만 잡혀서 하나은행만 노출됨.
--
-- 수정:
--   필터를 coalesce(repaid, false) = false AND status <> 'cancelled' 로 변경.
--   = 한도 점유 LC = "아직 상환 안 됐고, 취소되지도 않은 LC".
--
-- 영향:
--   - by_bank.used, totals.total_used_usd, by_company.used_usd 가 정확히 잡힘
--   - trend24.lc_open_usd / lc_open_count 24개월 추이도 동일 기준 정정
--   - maturity_filtered (만기 30일 알림) 의 동일 필터는 본 PR 에서는 건드리지 않음
--     (별도 검토 필요 — settled-but-not-repaid LC 만기 알림 표시 여부)
