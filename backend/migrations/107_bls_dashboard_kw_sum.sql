-- 107: bls_dashboard() RPC 에 trend24.kw_sum 추가 (월별 입고 kW 합계).
-- InventoryPage '흐름' 탭에서 월별 입고 / 재고 walking back 계산에 사용.
-- 080 의 마이그 + bl_line_items LATERAL join + trend_raw 에 SUM(kw_sum) 추가만 다름.
  ...
  WITH
    ...
    trend_raw AS (
      SELECT
        ...
        count(*) FILTER (WHERE status IN ('shipping', 'arrived'))::int AS shipping_count,
        count(*) FILTER (WHERE status = 'customs')::int AS customs_count,
        COALESCE(SUM(kw_sum), 0)::float8 AS kw_sum
      FROM base
    ...
    trend24_arr AS (
      SELECT
        ...
        COALESCE(t.kw_sum, 0)::float8 AS kw_sum
      FROM months m
