-- 076: outbounds_dashboard() RPC — 가장 복잡한 dashboard.
-- 운영 측정 outbounds/dashboard avg 1.7s / max 9.9s.
--
-- 응답 셰이프 OutboundDashboard struct 와 1:1:
--   { totals, trend24[], weekly12[], yoy3y, period, by_usage[], by_manufacturer_top10[],
--     by_customer_top10[], sale_conversion: { eligible_count, linked_count, monthly[],
--                                              by_usage[], by_manufacturer_top10[], by_customer_top10[] } }
--
-- weekly12 (12 주, 월요일 기준) 와 yoy3y (3년 동기 비교) 는 generate_series 로 처리.
-- sale_conversion 은 usage_category in (sale, sale_spare) 인 행만 대상.
...
BEGIN
  WITH
    ...
    -- 6) period 적용된 scoped (breakdowns 용).
    scoped AS (
      SELECT * FROM base
      WHERE
        (p_period = 'lifetime')
    ...
    by_cust_top10 AS (
      ...
      FROM by_cust_raw c, scoped_total st
    ...
    -- monthly: 24 month series, eligible/linked counts per month (outbound_date).
    sc_monthly_raw AS (
      SELECT
        to_char(outbound_date::date, 'YYYY-MM') AS month,
        count(*)::int AS eligible_count,
