...
// 비유: 창고 명패 옆에 "이 창고 — 입고(B/L) N건 · 출고 N건" 도장을 자동으로 찍어 돌려주는 것
func (h *WarehouseHandler) UsageCounts(w http.ResponseWriter, r *http.Request) {
	body, err := dbrpc.Call(r.Context(), "sf_warehouse_usage_counts", map[string]interface{}{})
	if err != nil {
		log.Printf("[창고 참조 건수 조회 실패] %v", err)
