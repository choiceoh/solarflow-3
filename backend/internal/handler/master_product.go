...
// 비유: 카탈로그 카드 옆에 "이 품번 — 매입라인 N건 · 출고 N건" 도장을 자동으로 찍어 돌려주는 것
func (h *ProductHandler) UsageCounts(w http.ResponseWriter, r *http.Request) {
	body, err := dbrpc.Call(r.Context(), "sf_product_usage_counts", map[string]interface{}{})
	if err != nil {
		log.Printf("[품번 참조 건수 조회 실패] %v", err)
