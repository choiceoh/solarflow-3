...
func TestRequireTenantScope_Topsolar(t *testing.T) {
	guard := RequireTenantScope(TenantScopeTopsolar, TenantScopeCable)
...
// TestRequireTenantScope_CableAllowed — cable 토큰은 module 계열 라우트를 통과한다(D-119)
func TestRequireTenantScope_CableAllowed(t *testing.T) {
	guard := RequireTenantScope(TenantScopeTopsolar, TenantScopeCable)

	req := httptest.NewRequest(http.MethodGet, "/api/v1/lcs", nil)
	req = req.WithContext(SetUserContext(req.Context(), "u3", "operator", "u3@solarflow.local", TenantScopeCable, nil))
	rec := httptest.NewRecorder()
	...
	guard(next).ServeHTTP(rec, req)
	if !called {
		t.Fatalf("cable 사용자는 module 계열 가드를 통과해야 합니다")
	}
	if rec.Code != http.StatusNoContent {
		t.Fatalf("기대 상태코드 204, 실제=%d body=%s", rec.Code, rec.Body.String())
	}
