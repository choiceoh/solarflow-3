...
use crate::model::price_forecast::{
    PriceForecastMarketSnapshot, PriceForecastObservation, PriceForecastRunInput,
    PriceForecastScenario, PriceForecastSourceQuality, PriceForecastStrategyRequest,
...
pub fn calculate_price_forecast_strategy(
    ...
    let cmm = latest_price_for(&req.observations, &["cmm_fob_china_topcon_600w"]);
    let floor = latest_price_for(&req.observations, &["cpia_cost_floor"]);
    let tender = latest_price_for(&req.observations, &["china_state_tender"]);
    let current = cmm
        .or_else(|| {
            latest_price_for(
                &req.observations,
    ...
    let cmm_trend_pct = trend_pct(&req.observations, "cmm_fob_china_topcon_600w");
    ...
    let source_quality = calculate_source_quality(req, now);
    ...
    let scenarios = build_scenarios(&req.observations, current, tender, floor, cmm_trend_pct);
    ...
    let basis = build_basis(&req.observations, cmm, tender, floor, current);
...
fn calculate_source_quality(
    ...
    let mut items = aggregates
        ...
        .map(|(source_key, aggregate)| {
            ...
            let score = clamp(
                confidence_score + recency_score + count_score - run_penalty,
                0.0,
                100.0,
            );
...
mod tests {
    ...
    fn observation(metric_key: &str, value_date: &str, price: f64) -> PriceForecastObservation {
        PriceForecastObservation {
            source_key: if metric_key == "cpia_cost_floor" {
                "cpia_floor"
            } else {
                "opis"
            }
            .to_string(),
            source_name: if metric_key == "cpia_cost_floor" {
                "CPIA"
            } else {
                "OPIS"
            }
